...
<style>
  ...
  body {
    background: url('http://goo.gl/ZHzmP');
  ...
  .bypass: hover {
    background-color: rgb(6, 118, 146);
    box-shadow: rgba(255, 255, 255, .75) 0px 0px 9px 1px;
  }
</style>
...
<center>
  <br />
  <br />
  <div class="head">Random Shit Bypasser</div>
  <br />
  <br />
  <form method="post">
    <textarea name="urllist"><?php echo htmlspecialchars($_POST['urllist'], ENT_QUOTES); ?></textarea>
    <br />
      ...
      foreach ($list as $a) {
        if (empty($a)) {
          ...
          if ($exp === "ShitHappen1") {
            ...
            <tr>
              <th>
                <font class="number"><?php echo $i++; ?></font>
              </th>
              <th>
                <font class="shittendstohappen">Can't bypass that link!</font>
              </th>
            </tr>
            <?php
          }
          elseif (strpos(strtolower(urldecode($exp)), "javascript:") !== false || $exp === "ShitHappen2") {
            ...
            <tr>
              ...
              <th>
                <font class="shittendstohappen">Link provide is offline or not exist!</font>
              </th>
            </tr>
            <?php
          } else {
            ...
            <tr>
              ...
              <th>
                <a href="<?php echo $exp; ?>"><font class="link"><?php echo $exp; ?></font></a>
              </th>
            </tr>
            <?php
          }
...
if ($CounT) {
  ?>
  <center>
    <div class='foot'>Number Of SHIT(s) Bypassed on
      <?php echo htmlentities($_SERVER[ "HTTP_HOST"], ENT_QUOTES, "utf-8"); ?> is 
      <b>
        <u><?php echo ' '.countLink(); ?></u>
      </b>
    </div>
  </center>
  <?php
...
function bypass($link) {
  ...
  if(strpos($get, "Location:")) {
    ...
    return $out[1][0];
  } else if (preg_match('/http(s?):\/\/(www\.)?(q.gs|adf.ly|j.gs|u.bb)\/.*/', $link) ) {
    preg_match_all('/var ysmm \= \'(.*?)\'\;/', $get, $out);
    return adfly_decode($out[1][0]);
  } else if (preg_match('/http(s?):\/\/(www\.)?.*\.(linkbucks.com|tinybucks.net|theseblogs.com)/', $link) ) {
    preg_match_all('/Lbjs\.TargetUrl \= \'(.*?)\'/', $get, $out);
    return $out[1][0];
  } else if (preg_match('/http(s?):\/\/(www\.)?go\.urlcash\.net\/.*/', $link)) {
    preg_match_all('/linkDestUrl \= \'(.*?)\'/', $get, $out);
    return $out[1][0];
  } else if (preg_match('/http(s?):\/\/(www\.)?lix\.in\/.*/', $link)) {
    preg_match('/name=\'tiny\' value=\'(.*?)\'/', $get, $out);
    $data = curl($link, "tiny=".$out[1]."&submit=continue&submit=submit");
    preg_match('/src\=\"(.*?)\"/', $data, $out);
    return $out[1];
  } else if (preg_match('/http(s?):\/\/(www\.)?anonym\.to\/\?.*/', $link)) {
    preg_match('/<p id=\"url\"><a href=\"(.*?)\"/', $get, $out);
    return $out[1];
  } else if (preg_match('/http(s?):\/\/(www\.)?anonymz\.com\/\?.*/', $link)) {
    preg_match('/<meta http-equiv=\"refresh\" content=\"0\; url=(.*)\">/', $get, $out);
    return $out[1];
  } else if (preg_match('/http(s?):\/\/(www\.)?adfoc\.us\/.*/', $link)) {
    preg_match('/[^\/\/]var click_url \= \"(.*?)\"\;/', $get, $out);
    return $out[1];
  } else if (preg_match('/http(s?):\/\/iic\.my\/.*/', $link)) {
    preg_match_all('/top\.location \= \'(.*?)\'/', $get, $out);
    return $out[1][0];
  } else if (preg_match('/http(s?):\/\/adfa\.st\/.*/', $link)) {
    preg_match('/<a href=\"(.*)\"><button/', $get, $out);
    return $out[1];
  } else if (preg_match('/http(s?):\/\/linkc\.at\/.*/', $link)) {
    $get = curl($link, "text=&image=");
    preg_match('/window\.location \= \'(.*)\'<\/script>/', $get, $out);
    return $out[1];
  } else if (preg_match('/http(s?):\/\/ad7\.biz\/.*/', $link)) {
    preg_match_all('/border\=0 src\=\"(.*)&url\=(.*)&ref\=/', $get, $out);
    return $out[2][0];
  } else if (preg_match('/t\.co/i', $link)) {
